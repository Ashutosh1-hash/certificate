...
console.log(csk);
            ...
            connection.connect((error) => {
                if (error) {
                    ...
                    let query = 'REPLACE INTO sample4(s_no, name,course,certificate_no,batch_date,email,token) VALUES ?';
...
app.get("/db1",(req,resp)=>{
  ...
  var sql = 'SELECT * FROM sample4 WHERE token = ?';
...
app.get('/table2',(req,res)=>{
  ...
  var connection = mysql.createConnection({
      ...
      user     : 'root',
      password : '',
      database : 'henry_harvin'*/
      host     : 'localhost',
      user     : 'root',
      password : '',
      database : 'henry_harvin2'
  }); 
